...
describe('migrationsDir', () => {
    ...
    beforeEach(() => {
        fs = mockFs();
        config = mockConfig();
        migrationsDir = proxyquire(
            '../../lib/env/migrationsDir',
            {
                'fs-extra': fs,
                './config': config,
            },
    ...
    describe('resolve()', () => {
        ...
        it('should use the default migrations directory when unable to read the config file', async () => {
            config.read.throws(
                new Error('Cannot read config file'),
            );
            expect(await migrationsDir.resolve()).to.equal(
                path.join(process.cwd(), 'migrations'),
            );
    ...
    describe('getFileNames()', () => {
        it('should read the directory and yield the result', async () => {
            fs.readdir.returns(
                Promise.resolve(['file1.js', 'file2.js']),
            );
            const files =
        ...
        it('should attempt to load the fileName in the migrations directory', async () => {
            ...
            try {
                ...
                expect(err.message).to.match(
                    new RegExp(
                        `Cannot find module '${pathToMigration}'`,
                    ),
                );
            }
        });
    ...
    describe('doesSampleMigrationExist()', () => {
        it('should return true if sample migration exists', async () => {
            fs.stat.returns(Promise.resolve());
            const result =
                await migrationsDir.doesSampleMigrationExist();
    ...
    describe('loadFileHash()', () => {
        it('should return a hash based on the file contents', async () => {
            fs.readFile.returns(
                Promise.resolve('some string to hash'),
            );
            const result = await migrationsDir.loadFileHash(
                'somefile.js',
            );
            expect(result).to.equal(
                'ea83a45637a9af470a994d2c9722273ef07d47aec0660a1d10afe6e9586801ac',
            );
        });
    });
});
