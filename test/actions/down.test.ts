describe('down', () => {
    ...
    function mockStatus() {
        return sinon.stub().returns(
            Promise.resolve([
                {
                    fileName:
                        '20160609113224-first_migration.js',
                    appliedAt: new Date(),
                },
                {
                    fileName:
    ...
    function mockMigrationsDir() {
        return {
            loadMigration: sinon
                .stub()
                .returns(Promise.resolve(migration)),
        };
    }
    ...
    it('should yield empty list when nothing to downgrade', async () => {
        status.returns(
            Promise.resolve([
                {
                    fileName:
                        '20160609113224-some_migration.js',
                    appliedAt: 'PENDING',
                },
            ]),
        );
    ...
    it('should remove the entry of the downgraded migration from the changelog collection', async () => {
        await down(db);
        expect(
            changelogCollection.deleteOne.called,
        ).to.equal(true);
        expect(
            changelogCollection.deleteOne.callCount,
        ).to.equal(1);
    });
