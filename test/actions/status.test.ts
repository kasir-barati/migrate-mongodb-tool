describe('status', () => {
                    ...
                    switch (fileName) {
                        case '20160509113224-first_migration.js':
                            return Promise.resolve(
                                '0f295f21f63c66dc78d8dc091ce3c8bab8c56d8b74fb35a0c99f6d9953e37d1a',
                            );
                        case '20160512091701-second_migration.js':
                            return Promise.resolve(
                                '18b4d9c95a8678ae3a6dd3ae5b8961737a6c3dd65e3e655a5f5718d97a0bff70',
                            );
                        case '20160513155321-third_migration.js':
                            return Promise.resolve(
                                '1f9eb3b5eb70b2fb5b83fa0c660d859082f0bb615e835d29943d26fb0d352022',
                            );
                        default:
                            return Promise.resolve();
                    }
    ...
    function mockFs() {
        return {
            ...
            readFile: sinon
                .stub()
                .returns(
                    Promise.resolve('some file content'),
                ),
        };
    }
    ...
    it('should check that the migrations directory exists', async () => {
        await status(db);
        expect(migrationsDir.shouldExist.called).to.equal(
            true,
        );
    });
    ...
    it('should get the list of files in the migrations directory', async () => {
        await status(db);
        expect(migrationsDir.getFileNames.called).to.equal(
            true,
        );
    });
    ...
    it('should fetch the content of the changelog collection', async () => {
        await status(db);
        expect(changelogCollection.find.called).to.equal(
            true,
        );
        expect(
            changelogCollection.find({}).toArray.called,
        ).to.equal(true);
    });
    ...
    it('it should mark changed scripts with pending', async () => {
        ...
        migrationsDir.loadFileHash.callsFake((fileName) => {
            switch (fileName) {
                ...
                case '20160512091701-second_migration.js':
                    return Promise.resolve(
                        '18b4d9c95a8678ae3a6dd3ae5b8961737a6c3dd65e3e655a5f5718d97a0bff71',
                    );
                case '20160513155321-third_migration.js':
                    return Promise.resolve(
                        '1f9eb3b5eb70b2fb5b83fa0c660d859082f0bb615e835d29943d26fb0d352022',
        ...
        expect(statusItems).to.deep.equal([
            ...
            },
            {
                ...
                fileName:
                    '20160512091701-second_migration.js',
                fileHash:
                    '18b4d9c95a8678ae3a6dd3ae5b8961737a6c3dd65e3e655a5f5718d97a0bff71', // this hash is different
            },
            {
                appliedAt: 'PENDING',
                fileName:
