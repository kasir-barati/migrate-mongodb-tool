describe('up', () => {
    ...
    function mockStatus() {
        return sinon.stub().returns(
            Promise.resolve([
                {
                    fileName:
                        '20160605123224-first_applied_migration.js',
                    appliedAt: new Date(),
                },
                {
                    fileName:
                        '20160606093207-second_applied_migration.js',
                    appliedAt: new Date(),
                },
                {
                    fileName:
    ...
    function mockMigrationsDir() {
        ...
        mock.loadMigration
            ...
            .returns(
                Promise.resolve(firstPendingMigration),
            );
        mock.loadMigration
            .withArgs(
                '20160608060209-second_pending_migration.js',
            ...
            .returns(
                Promise.resolve(secondPendingMigration),
            );
        return mock;
    }
    ...
    it('should load all the pending migrations', async () => {
        await up(db);
        expect(migrationsDir.loadMigration.called).to.equal(
            true,
        );
        expect(
            migrationsDir.loadMigration.callCount,
        ).to.equal(2);
        expect(
            migrationsDir.loadMigration.getCall(0).args[0],
        ).to.equal(
            '20160607173840-first_pending_migration.js',
        );
        expect(
            migrationsDir.loadMigration.getCall(1).args[0],
        ).to.equal(
            '20160608060209-second_pending_migration.js',
        );
    });
    ...
    it('should upgrade all pending migrations in ascending order', async () => {
        await up(db);
        expect(firstPendingMigration.up.called).to.equal(
            true,
        );
        expect(secondPendingMigration.up.called).to.equal(
            true,
        );
        sinon.assert.callOrder(
            firstPendingMigration.up,
            secondPendingMigration.up,
        );
    });
    ...
    it('should populate the changelog with info about the upgraded migrations', async () => {
        ...
        expect(
            changelogCollection.insertOne.called,
        ).to.equal(true);
        expect(
            changelogCollection.insertOne.callCount,
        ).to.equal(2);
        expect(
            changelogCollection.insertOne.getCall(0)
                .args[0],
        ).to.deep.equal({
    ...
    it('should stop migrating when an error occurred and yield the error', async () => {
        secondPendingMigration.up.returns(
    ...
    it('should yield an error + items already migrated when unable to update the changelog', async () => {
        changelogCollection.insertOne
            .onSecondCall()
            .returns(
                Promise.reject(new Error('Kernel panic')),
            );
        try {
            await up(db);
            expect.fail('Error was not thrown');
